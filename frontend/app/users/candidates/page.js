...
import {
  User, Search, Trash2, Edit3, X, Check, Loader2,
  Users, Mail, Calendar, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  AlertCircle, Code2, Award, ExternalLink, XCircle,
...
export default function CandidatesPage() {
  ...
  const fetchCandidates = async (pageNum = 1) => {
    ...
    setError(null);
    try {
      const data = await userDirectoryService.getUsersByRole('CANDIDATE', { search, page: pageNum, limit: 12 });
  ...
  const avgSkills = useMemo(() => {
    const withSkills = candidates.filter(c => (c.candidateProfile?.skills?.length || 0) > 0);
    if (!withSkills.length) return 0;
    const total = withSkills.reduce((sum, c) => sum + (c.candidateProfile?.skills?.length || 0), 0);
    return Math.round(total / withSkills.length);
  }, [candidates]);

  // ── Edit handlers ──
  const startEdit = (c) => { setEditingId(c.id); setEditForm({ name: c.name || '' }); };
  const cancelEdit = () => { setEditingId(null); setEditForm({ name: '' }); };

  const saveEdit = async (id) => {
    setSaving(true);
    setError(null);
    try {
      await userDirectoryService.updateUser(id, { name: editForm.name });
      setSuccessMsg('Candidate updated!');
      setEditingId(null);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50 dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-emerald-500/5 dark:bg-emerald-500/5 blur-[120px]" />
        <div className="absolute -bottom-40 -left-40 w-[400px] h-[400px] rounded-full bg-teal-500/5 dark:bg-teal-500/5 blur-[100px]" />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div className="animate-fade-in">
            <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-2">
              ...
              <span className="text-emerald-600 dark:text-emerald-400 font-medium">Candidates</span>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 animate-fade-in">
          <StatChip icon={Users} label="Total" value={totalCount} gradient="from-emerald-500 to-teal-500" />
          <StatChip icon={UserPlus} label="New this month" value={newThisMonth} gradient="from-blue-500 to-cyan-500" />
          <StatChip icon={Activity} label="Avg. Skills" value={avgSkills} gradient="from-violet-500 to-purple-500" />
          <StatChip icon={TrendingUp} label="Pages" value={totalPages} gradient="from-amber-500 to-orange-500" />
        </div>
        ...
        <form onSubmit={handleSearch} className="mb-6 animate-fade-in">
          <div className="relative max-w-lg group">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 bg-white/80 dark:bg-slate-800/70 border-2 border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 outline-none transition-all duration-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 hover:border-slate-300 dark:hover:border-slate-600"
            />
            {search && (
              <button
                type="button"
        ...
        {loading ? (
          ...
          <div className="text-center py-20 animate-fade-in">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-100 to-teal-100 dark:from-emerald-900/30 dark:to-teal-900/30 flex items-center justify-center mx-auto mb-5 shadow-inner">
              <Users className="w-10 h-10 text-emerald-400 dark:text-emerald-500" />
            </div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
              {search
                ...
                : 'Candidates will appear here once they register and complete their profiles.'}
            </p>
            {search && (
              <button
                onClick={clearSearch}
                className="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20"
              >
                <X className="w-4 h-4" />
                Clear search
              </button>
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Showing <span className="font-semibold text-slate-700 dark:text-slate-300">{candidates.length}</span> of{' '}
                <span className="font-semibold text-slate-700 dark:text-slate-300">{totalCount}</span> candidates
              </p>
                ...
                return (
                  <div
                    ...
                    className={`group relative overflow-hidden bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border-2 transition-all duration-300 ${
                      isEditing
                        ? 'ring-2 ring-emerald-500/30 border-emerald-300 dark:border-emerald-700 shadow-lg shadow-emerald-500/5'
                        : isDeleting
                        ? 'ring-2 ring-red-500/30 border-red-300 dark:border-red-700'
                        : 'border-slate-200/60 dark:border-slate-700/60 hover:border-emerald-300 dark:hover:border-emerald-700/60 hover:shadow-lg hover:shadow-emerald-500/5'
                    }`}
                  >
                    {/* Top accent line */}
                    <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${
                      isEditing
                        ...
                        : 'from-emerald-400/0 via-emerald-400/0 to-emerald-400/0 group-hover:from-emerald-400 group-hover:via-teal-400 group-hover:to-emerald-400'
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-4 min-w-0 flex-1">
                          ...
                          <Link
                            href={`/users/${c.id}`}
                            className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all duration-300"
                          >
                            {c.name?.charAt(0) || 'C'}
                          </Link>
                            ...
                            {isEditing ? (
                              <div className="space-y-2 animate-fade-in">
                                <input
                                  ...
                                  className="w-full px-3 py-2 bg-white dark:bg-slate-700 border-2 border-slate-200 dark:border-slate-600 rounded-lg text-sm font-semibold text-slate-900 dark:text-white outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all"
                              ...
                              <>
                                ...
                                >
                                  <h3 className="text-base font-bold text-slate-900 dark:text-white group-hover/link:text-emerald-600 dark:group-hover/link:text-emerald-400 transition-colors">
                                    {c.name}
                                  </h3>
                                  <ExternalLink className="w-3.5 h-3.5 text-slate-300 group-hover/link:text-emerald-400 transition-all opacity-0 group-hover/link:opacity-100 -ml-1 group-hover/link:ml-0" />
                                </Link>
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5 text-sm text-slate-500 dark:text-slate-400">
                                  ...
                                  {profile.title && (
                                    <span className="flex items-center gap-1.5">
                                      ...
                                      <span className="truncate max-w-[180px]">{profile.title}</span>
                                    </span>
                                  )}
                                  <span className="flex items-center gap-1.5">
                                ...
                                {skills.length > 0 && (
                                  <div className="flex flex-wrap gap-1.5 mt-2">
                                    {skills.slice(0, 4).map((s, i) => (
                                      <span
                                        key={i}
                                        className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-xs font-medium text-emerald-600 dark:text-emerald-400 border border-emerald-200/50 dark:border-emerald-800/30"
                                      >
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <Link
                            href={`/users/${c.id}`}
                            className="p-2 rounded-lg text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all opacity-0 group-hover:opacity-100 -mr-1"
                            title="View Profile"
                          >
                          ...
                          {isDeleting ? (
                            ...
                            <div className="flex items-center gap-1 animate-fade-in">
                              <button
                                ...
                                disabled={saving}
                                className="px-3 py-1.5 rounded-lg bg-emerald-500 text-white hover:bg-emerald-600 transition-all text-xs font-bold disabled:opacity-50 shadow-sm"
                              >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-200 dark:border-slate-700/60 animate-fade-in">
            ...
            <div className="flex items-center gap-1 bg-white/80 dark:bg-slate-800/50 rounded-xl border border-slate-200/60 dark:border-slate-700/60 p-1 shadow-sm">
              <button
                ...
                disabled={page === 1}
                className="p-2 rounded-lg text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                title="First"
              >
